//
//  NSDictionary+JoySafe.m
//  Pods
//
//  Created by Joymake on 2016/5/14.
//

#import "NSDictionary+JoySafe.h"
#import <objc/runtime.h>
#import "NSObject+JoyCategory.h"

@implementation NSDictionary (JoySafe)
...
@implementation NSMutableDictionary (JoySafe)

#ifdef RELEASE
+(void)load{
    [super load];
    [objc_getClass("__NSDictionaryM") swizzleMethod:@selector(setObject:forKey:) swizzledSelector:@selector(joySetObject:forKey:)];
}

-(void)joySetObject:(id)anObject forKey:(id<NSCopying>)aKey{
    
    if (!anObject){
        @try {
            [self joySetObject:anObject forKey:aKey];
        }
